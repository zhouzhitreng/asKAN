...
# 360 parameters
# total_params = sum(p.numel() for p in Bessel_model.parameters() if p.requires_grad)
# total_params
# In[1]:
# 
...
class BesselKAN(nn.Module):
    def __init__(self):
        ...
        self.Besselkan1 = BesselKANLayer(2, 30, 3)
        self.Besselkan2 = BesselKANLayer(30, 1, 3)
...
x_train1_grid, x_train2_grid = torch.meshgrid(x_train1, x_train2, indexing='xy')  # 形状 (400, 400)
x_train_all = torch.stack([x_train1_grid.reshape(-1), x_train2_grid.reshape(-1)], dim=1)  # (160000, 2)
...
optimizer_Bessel = LBFGS(Bessel_model.parameters(), lr=0.5, history_size=10, line_search_fn="strong_wolfe", tolerance_grad=1e-32, tolerance_change=1e-32, tolerance_ys=1e-32)
...
for epochout in range(epochouts):
    for epoch in range(epochs):
        ...
        optimizer_Bessel.step(closure_Bessel).clone().detach()  
        ...
        train_error[epochout,epoch] = criterion(Bessel_model(x_train), y_train)
        test_error[epochout,epoch] = criterion(Bessel_model(x_test0), y_test0)
        
        if epoch % 5 == 0:
            with torch.no_grad():
                current_loss_Bessel = criterion(Bessel_model(x_train), y_train)
                current_loss_Bessel_test = criterion(Bessel_model(x_test0), y_test0)
                current_loss_mlp = criterion(mlp_model(x_train), y_train)
            print(f'Epoch {epoch + 1}/{epochs}, '
                  f'BesselKAN Train Loss : {current_loss_Bessel.item():.5f}, '
                  f'BesselKAN Test Loss: {current_loss_Bessel_test.item():.5f}, '
                  f'MLP Loss: {current_loss_mlp.item():.5f}')
    dataset, x0, x1, u1,corall = get_direction(Bessel_model, dataset)
    ...
    if epochout == 0:
        u_rot = u1.cpu().clone() 
        Bessel_model0 = copy.deepcopy(Bessel_model) 
...
# prediction
y_pred_Bessel = Bessel_model(x_test).detach().reshape(400, 400).numpy()  
y_pred_mlp = mlp_model(x_test).detach().reshape(400, 400).numpy()          
y_true = target_function(x_test).detach().reshape(400, 400).numpy()         

# figure
fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(12, 3), sharey=True)

# BesselKAN prediction
im1 = ax1.contourf(x1_grid.numpy(), x2_grid.numpy(), y_pred_Bessel, levels=20, vmin=-0.9, vmax=0.9, cmap='viridis')
ax1.set_title("BesselKAN Predictions")
ax1.set_xlabel("x1")
ax1.set_ylabel("x2")
