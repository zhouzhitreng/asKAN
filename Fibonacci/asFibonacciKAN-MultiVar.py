...
# total_params = sum(p.numel() for p in Fibonacci_model.parameters() if p.requires_grad)
# total_params
# In[1]:
# 
...
class FibonacciKAN(nn.Module):
    def __init__(self):
        ...
        self.Fibonaccikan1 = FibonacciKANLayer(2, 30, 3)
        self.Fibonaccikan2 = FibonacciKANLayer(30, 1, 3)
...
x_train = x_train_all[random_indices]  # 形状 (16000, 2)
...
optimizer_Fibonacci = LBFGS(Fibonacci_model.parameters(), lr=0.5, history_size=10, line_search_fn="strong_wolfe", tolerance_grad=1e-32, tolerance_change=1e-32, tolerance_ys=1e-32)
...
for epochout in range(epochouts):
    for epoch in range(epochs):
        ...
        def closure_Fibonacci():
            ...
            return loss_Fibonacci 
        
        optimizer_Fibonacci.step(closure_Fibonacci).clone().detach()  
        ...
        train_error[epochout,epoch] = criterion(Fibonacci_model(x_train), y_train)
        test_error[epochout,epoch] = criterion(Fibonacci_model(x_test0), y_test0)
      
        if epoch % 5 == 0:
            with torch.no_grad():
                current_loss_Fibonacci = criterion(Fibonacci_model(x_train), y_train)
                current_loss_Fibonacci_test = criterion(Fibonacci_model(x_test0), y_test0)
                current_loss_mlp = criterion(mlp_model(x_train), y_train)
            print(f'Epoch {epoch + 1}/{epochs}, '
                  f'FibonacciKAN Train Loss: {current_loss_Fibonacci.item():.5f}, '
                  f'FibonacciKAN Loss Test: {current_loss_Fibonacci_test.item():.5f}, '
                  f'MLP Loss: {current_loss_mlp.item():.5f}')
    dataset, x0, x1, u1,corall = get_direction(Fibonacci_model, dataset)
    ...
    if epochout == 0:
        u_rot = u1.cpu().clone() 
        Fibonacci_model0 = copy.deepcopy(Fibonacci_model)
...
# 模型预测
y_pred_Fibonacci = Fibonacci_model(x_test).detach().reshape(400, 400).numpy()  # FibonacciKAN 
...
# FibonacciKAN prediction
im1 = ax1.contourf(x1_grid.numpy(), x2_grid.numpy(), y_pred_Fibonacci, levels=20, vmin=-0.9, vmax=0.9, cmap='viridis')
ax1.set_title("FibonacciKAN Predictions")
ax1.set_xlabel("x1")
ax1.set_ylabel("x2")
