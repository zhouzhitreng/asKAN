...
def B_batch(x, grid, k=0, extend=True, device='cpu'):
    ...
    -----
        x : 2D torch.tensor
            inputs, shape (number of splines, number of samples) ?? 写错了(batch, number of samples)
    ...
    if k == 0:
        ...
        B_km1 = B_batch_0(x[:,:,0], grid=grid[0], k=k - 1)
        
        value = (x - grid[:, :, :-(k + 1)]) / (grid[:, :, k:-1] - grid[:, :, :-(k + 1)]) * B_km1[:, :, :-1] + (
                    grid[:, :, k + 1:] - x) / (grid[:, :, k + 1:] - grid[:, :, 1:(-k)]) * B_km1[:, :, 1:]        
    
    #value = torch.cos(value)+torch.sin(value)
...
def B_batch_0(x, grid, k=0, extend=True, device='cpu'):
    '''
    evaludate x on B-spline bases
...
#     xx = x - x
#     dgrid = grid[0,1,2] - grid[0,1,1]
#     #value = xx + torch.cos(2*torch.pi*(grid[:, :, :-(k+1)]+grid[0, 1, 0])/dgrid)
#     value = xx + torch.cos(2*torch.pi*(grid[:, :, :-(k+1)]+grid[0, 1, 0])/dgrid)
#     #print(value.shape)
#         # value的第一维为样本的个数，第二维为sample的个数，第三维度为每个节点的函数值需要乘上的数字
