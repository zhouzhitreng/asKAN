...
def create_dataset_zzt(namedata, 
                   n_var=2, 
                   ranges = [-1,1],
                   train_num=1000, 
                   test_num=1000,
    ...
    data = scipy.io.loadmat(namedata)  # 请确保文件路径和文件名正确

    # Step 2: Extract the data and labels arrays
    ...
    #data_array, labels_array, test_size=0.01, random_state=42)
    
    #下面这样就是随机的
    train_data, test_data, train_labels, test_labels = train_test_split(
    data_array, labels_array, test_size=0.2)
