...
sol_fun = lambda x: torch.sin(2*torch.pi*x[:,[0]])*torch.sin(2*torch.pi*x[:,[1]])
source_fun = lambda x: -2*4*torch.pi**2 * torch.sin(2*torch.pi*x[:,[0]])*torch.sin(2*torch.pi*x[:,[1]])

dim = 2
np_i = 31  # number of inner points
np_b = 31  # number of boundary points
ranges = [-1, 1]
