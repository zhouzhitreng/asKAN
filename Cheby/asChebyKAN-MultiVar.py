...
# total_params = sum(p.numel() for p in Cheby_model.parameters() if p.requires_grad)
# total_params
# In[1]:
# 
...
class ChebyKAN(nn.Module):
    def __init__(self):
        ...
        self.Chebykan1 = ChebyKANLayer(2, 30, 3)
        self.Chebykan2 = ChebyKANLayer(30, 1, 3)
...
optimizer_Cheby = LBFGS(Cheby_model.parameters(), lr=0.5, history_size=10, line_search_fn="strong_wolfe", tolerance_grad=1e-32, tolerance_change=1e-32, tolerance_ys=1e-32)
...
for epochout in range(epochouts):
    for epoch in range(epochs):
        ...
        optimizer_Cheby.step(closure_Cheby).clone().detach() 
        ...
        train_error[epochout,epoch] = criterion(Cheby_model(x_train), y_train)
        test_error[epochout,epoch] = criterion(Cheby_model(x_test0), y_test0)
        # --- print ---
        if epoch % 5 == 0:
            with torch.no_grad():
                current_loss_Cheby = criterion(Cheby_model(x_train), y_train)
                current_loss_Cheby_test = criterion(Cheby_model(x_test0), y_test0)
                current_loss_mlp = criterion(mlp_model(x_train), y_train)
            print(f'Epoch {epoch + 1}/{epochs}, '
                  f'ChebyKAN Train Loss: {current_loss_Cheby.item():.5f}, '
                  f'ChebyKAN Test Loss : {current_loss_Cheby_test.item():.5f}, '
                  f'MLP Loss: {current_loss_mlp.item():.5f}')
    dataset, x0, x1, u1,corall = get_direction(Cheby_model, dataset)
    ...
    if epochout == 0:
        u_rot = u1.cpu().clone() 
        Cheby_model0 = copy.deepcopy(Cheby_model) 
...
# 模型预测
y_pred_Cheby = Cheby_model(x_test).detach().reshape(400, 400).numpy()  # ChebyKAN
...
# ChebyKAN prediction
im1 = ax1.contourf(x1_grid.numpy(), x2_grid.numpy(), y_pred_Cheby, levels=20, vmin=-0.9, vmax=0.9, cmap='viridis')
ax1.set_title("ChebyKAN Predictions")
ax1.set_xlabel("x1")
ax1.set_ylabel("x2")
