...
# total_params = sum(p.numel() for p in Legendre_model.parameters() if p.requires_grad)
# total_params
# In[1]:
# 
...
class LegendreKAN(nn.Module):
    def __init__(self):
        ...
        self.Legendrekan1 = LegendreKANLayer(2, 30, 3)
        self.Legendrekan2 = LegendreKANLayer(30, 1, 3)
...
optimizer_Legendre = LBFGS(Legendre_model.parameters(), lr=0.5, history_size=10, line_search_fn="strong_wolfe", tolerance_grad=1e-32, tolerance_change=1e-32, tolerance_ys=1e-32)
...
for epochout in range(epochouts):
    for epoch in range(epochs):
        ...
        def closure_Legendre():
            ...
            return loss_Legendre 
        
        optimizer_Legendre.step(closure_Legendre).clone().detach()  
        ...
        train_error[epochout,epoch] = criterion(Legendre_model(x_train), y_train)
        test_error[epochout,epoch] = criterion(Legendre_model(x_test0), y_test0)
      
        if epoch % 5 == 0:
            with torch.no_grad():
                current_loss_Legendre = criterion(Legendre_model(x_train), y_train)
                current_loss_Legendre_test = criterion(Legendre_model(x_test0), y_test0)
                current_loss_mlp = criterion(mlp_model(x_train), y_train)
            print(f'Epoch {epoch + 1}/{epochs}, '
                  f'LegendreKAN Train Loss: {current_loss_Legendre.item():.5f}, '
                  f'LegendreKAN Loss Test: {current_loss_Legendre_test.item():.5f}, '
                  f'MLP Loss: {current_loss_mlp.item():.5f}')
    dataset, x0, x1, u1,corall = get_direction(Legendre_model, dataset)
    ...
    if epochout == 0:
        u_rot = u1.cpu().clone() 
        Legendre_model0 = copy.deepcopy(Legendre_model)
...
# 模型预测
y_pred_Legendre = Legendre_model(x_test).detach().reshape(400, 400).numpy()  # LegendreKAN 
...
# LegendreKAN prediction
im1 = ax1.contourf(x1_grid.numpy(), x2_grid.numpy(), y_pred_Legendre, levels=20, vmin=-0.9, vmax=0.9, cmap='viridis')
ax1.set_title("LegendreKAN Predictions")
ax1.set_xlabel("x1")
ax1.set_ylabel("x2")
