...
# total_params = sum(p.numel() for p in Hermite_model.parameters() if p.requires_grad)
# total_params
# In[1]:
# 
...
class HermiteKAN(nn.Module):
    def __init__(self):
        ...
        self.Hermitekan1 = HermiteKANLayer(2, 30, 3)
        self.Hermitekan2 = HermiteKANLayer(30, 1, 3)
...
optimizer_Hermite = LBFGS(Hermite_model.parameters(), lr=0.5, history_size=10, line_search_fn="strong_wolfe", tolerance_grad=1e-32, tolerance_change=1e-32, tolerance_ys=1e-32)
...
for epochout in range(epochouts):
    for epoch in range(epochs):
        ...
        optimizer_Hermite.step(closure_Hermite).clone().detach()  
        ...
        train_error[epochout,epoch] = criterion(Hermite_model(x_train), y_train)
        test_error[epochout,epoch] = criterion(Hermite_model(x_test0), y_test0)
      
        if epoch % 5 == 0:
            with torch.no_grad():
                current_loss_Hermite = criterion(Hermite_model(x_train), y_train)
                current_loss_Hermite_test = criterion(Hermite_model(x_test0), y_test0)
                current_loss_mlp = criterion(mlp_model(x_train), y_train)
            print(f'Epoch {epoch + 1}/{epochs}, '
                  f'HermiteKAN Train Loss: {current_loss_Hermite.item():.5f}, '
                  f'HermiteKAN Loss Test: {current_loss_Hermite_test.item():.5f}, '
                  f'MLP Loss: {current_loss_mlp.item():.5f}')
    dataset, x0, x1, u1,corall = get_direction(Hermite_model, dataset)
    ...
    if epochout == 0:
        u_rot = u1.cpu().clone() 
        Hermite_model0 = copy.deepcopy(Hermite_model)
...
# 模型预测
y_pred_Hermite = Hermite_model(x_test).detach().reshape(400, 400).numpy()  # HermiteKAN 
...
# HermiteKAN prediction
im1 = ax1.contourf(x1_grid.numpy(), x2_grid.numpy(), y_pred_Hermite, levels=20, vmin=-0.9, vmax=0.9, cmap='viridis')
ax1.set_title("HermiteKAN Predictions")
ax1.set_xlabel("x1")
ax1.set_ylabel("x2")
